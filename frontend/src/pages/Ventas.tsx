...
const Ventas = () => {
  ...
  // --- PRODUCTOS VENDIDOS ---
  const productosMap = new Map<string, { nombre: string, categoria: string, cantidad: number }>()
  ventasFiltradas.forEach(v => {
    v.detalles?.forEach((d: any) => {
      const key = `${d.producto_nombre}-${d.categoria_nombre || 'Sin categoría'}`
      if (!productosMap.has(key)) {
        productosMap.set(key, { nombre: d.producto_nombre, categoria: d.categoria_nombre || 'Sin categoría', cantidad: d.cantidad })
      } else {
  ...
  let productosVendidos = Array.from(productosMap.values())
      ...
      <div className="space-y-6">
        ...
        <div className="flex gap-4 mb-4">
          <button
              ...
              className={`px-4 py-2 rounded ${tab === 'ventas' ? 'bg-primary-600 text-white' : 'bg-gray-200 text-gray-700'}`}
          >
            Ventas
          </button>
          <button
              onClick={() => setTab('productosVendidos')}
              className={`px-4 py-2 rounded ${tab === 'productosVendidos' ? 'bg-primary-600 text-white' : 'bg-gray-200 text-gray-700'}`}
          >
            Productos Vendidos
          </button>
        </div>
                  ...
                  <table className="min-w-full divide-y divide-gray-200">
                    ...
                    {ventasFiltradas.map((venta) => (
                        <tr key={venta.id}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{venta.fecha ? format(new Date(venta.fecha), 'dd/MM/yyyy HH:mm', { locale: es }) : '-'}</td>
                    ...
                    <div className="relative top-20 mx-auto p-5 border w-3/4 max-w-4xl shadow-lg rounded-md bg-white">
                      <div className="mt-3">
                        <div className="flex justify-between items-center mb-4">
                          <h3 className="text-lg font-medium text-gray-900">Detalles de Venta #{selectedVenta.id}</h3>
                        ...
                        <div className="grid grid-cols-2 gap-6 mb-6">
                          <div>
                            ...
                            <div className="space-y-2 text-sm">
                              <div><span className="font-medium">Cliente:</span> {selectedVenta.cliente_nombre || 'No especificado'}</div>
                              <div><span className="font-medium">Fecha:</span> {selectedVenta.fecha ? format(new Date(selectedVenta.fecha), 'dd/MM/yyyy HH:mm', { locale: es }) : '-'}</div>
                              <div><span className="font-medium">Estado:</span> {selectedVenta.estado || 'Completada'}</div>
                            </div>
                          </div>
                          <div>
                            <h4 className="font-medium text-gray-900 mb-2">Resumen</h4>
                            <div className="text-2xl font-bold text-primary-600">{formatPrice(selectedVenta.total.toLocaleString())}</div>
                            <div className="text-sm text-gray-500">Total de la venta</div>
                          </div>
                        </div>
                          ...
                          <div className="overflow-x-auto">
                            <table className="min-w-full divide-y divide-gray-200">
                              ...
                              {selectedVenta.detalles?.map((detalle: any, index: number) => (
                                  <tr key={index}>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{detalle.producto_nombre}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{detalle.cantidad}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{formatPrice(detalle.precio_unitario)}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{formatPrice(detalle.subtotal)}</td>
                                  </tr>
                              ))}
                              </tbody>
                            </table>
                                ...
                                <table className="min-w-full divide-y divide-gray-200">
                                  ...
                                  {selectedVenta.pagosParciales.map((pago: any) => (
                                      <tr key={pago.id}>
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{format(new Date(pago.fecha), 'dd/MM/yyyy HH:mm', { locale: es })}</td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{formatPrice(pago.total)}</td>
