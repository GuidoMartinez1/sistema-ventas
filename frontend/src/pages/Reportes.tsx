...
// Helper: Calcular primer día del mes actual
...
const Reportes = () => {
    ...
    // CAMBIO: Inicializa fechaDesde con el primer día del mes actual por defecto
    ...
    const filtrarPorFecha = <T extends { fecha?: string }>(items: T[]) => {
        return items.filter(item => {
            ...
            // Se usa en-CA para obtener formato YYYY-MM-DD local consistente
            const fechaStr = fechaItem.toLocaleDateString('en-CA')
    ...
    // Se mantiene la funcionalidad original de limpiar todo (mostrar histórico completo)
    const limpiarFechas = () => {
        setFechaDesde('')
        setFechaHasta('')
        setFiltroMetodoPago('')
